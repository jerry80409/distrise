...
public class Bech32 {
  ...
  public static String toBech32(Bech32Prefix hrp, byte[] hexKey) throws CryptoException {
    var data = convertBits(hexKey, 8, 5, true);

    return Bech32.encode(Bech32.Encoding.BECH32, hrp.getCode(), data);
  }

  public static String toBech32(Bech32Prefix hrp, String hexKey) throws CryptoException {
    return toBech32(hrp, CryptoUtils.hexToBytes(hexKey));
  }

  // Added by squirrel
  public static String fromBech32(String strBech32) throws CryptoException {
   ...
   */
  public static String encode(final Bech32Data bech32) throws CryptoException {
  ...
  public static String encode(Encoding encoding, String hrp, final byte[] values) throws CryptoException {
    if (hrp.length() < 1) {
      throw new CryptoException("Human-readable part is too short");
    }
   ...
   */
  // Modified to throw NostrExceptions
  public static Bech32Data decode(final String str) throws CryptoException {
    boolean lower = false, upper = false;
    if (str.length() < 8) {
      throw new CryptoException("Input too short: " + str.length());
    }
    for (int i = 0; i < str.length(); ++i) {
      char c = str.charAt(i);
      if (c < 33 || c > 126) {
        throw new CryptoException(String.format("Invalid Character %c, %d", c, i));
      }
      if (c >= 'a' && c <= 'z') {
        if (upper) {
          throw new CryptoException(String.format("Invalid Character %c, %d", c, i));
    ...
    if (pos < 1) {
      throw new CryptoException("Missing human-readable part");
    }
    final int dataPartLength = str.length() - 1 - pos;
    if (dataPartLength < 6) {
      throw new CryptoException(String.format("Data part too short: %d)", dataPartLength));
    }
    byte[] values = new byte[dataPartLength];
    for (int i = 0; i < dataPartLength; ++i) {
      char c = str.charAt(i + pos + 1);
      if (CHARSET_REV[c] == -1) {
        throw new CryptoException(String.format("Invalid Character %c, %d", c, i + pos + 1));
      }
    ...
    if (encoding == null) {
      throw new CryptoException("InvalidChecksum");
    }
